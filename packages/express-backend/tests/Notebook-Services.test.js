...
test("add and delete new notebook and user --success", () => {
  ...
  return UserServices.addUser(newUser)
    .then((addedUser) => {
      ...
      return NotebookServices.addNotebook(sampleNotebook)
        .then((addedNotebook) => {
          expect(addedNotebook).toBeDefined();
          NotebookServices.notebookDelete(addedNotebook._id)
          ...
          return UserServices.userDelete(addedUser._id)
            .then((deletedUser) => {
...
test("find notebook by _id #3 --success", async () => {
  try {
    ...
    const foundNotebook =
      await NotebookServices.findNotebookById(
        addedNotebook._id
      );
...
test("find notebook by _id, invalid _id --fail", () => {
  return NotebookServices.findNotebookById("fakeid")
    .then(() => {
      throw new Error(
        "Expected findNotebookById to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("find notebook by user, invalid user --fail", () => {
  return NotebookServices.findNotebookByUserId("fakeid")
    .then(() => {
      throw new Error(
        "Expected findNotebookByUserId to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("find notebook by user #2 --success", async () => {
  try {
    ...
    const foundNotebook =
      await NotebookServices.findNotebookByUserId(
        addedNotebook.user
      );
    expect(foundNotebook).toBeDefined();
...
test("find notebook by user and key, invalid user --fail", () => {
  return NotebookServices.findNotebookByUserIdAndKey(
    "fakeid",
    "test"
  )
    .then(() => {
      throw new Error(
        "Expected findNotebookByUserAndKeyId to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("find notebook by user and key --success", async () => {
  try {
    ...
    // Step 3: Find the added notebook by user
    const foundNotebook =
      await NotebookServices.findNotebookByUserIdAndKey(
        addedNotebook.user,
        "Sample"
      );
    expect(foundNotebook).toBeDefined();
...
test("find notebook by user and key, user not found -- fail", () => {
  return NotebookServices.findNotebookByUserIdAndKey(
    ...
    .then(() => {
      throw new Error(
        "Expected findNotebookByUserIdAndKey to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("delete notebook, invalid id --fail", () => {
  return NotebookServices.notebookDelete("fakeid")
    .then(() => {
      throw new Error(
        "Expected deleteNotebook to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("delete notebook, not found --fail", () => {
  return NotebookServices.notebookDelete(
    "59e3ccd501154dcbf5f878b5"
  )
    .then(() => {
      //fake but valid id
