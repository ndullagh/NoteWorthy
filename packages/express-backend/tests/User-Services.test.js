...
test("delete user, invalid id --fail", () => {
  return UserServices.userDelete("fakeid")
    .then(() => {
      throw new Error(
        "Expected userDelete to fail but it succeeded"
      );
    })
    .catch((error) => {
...
test("delete user, that does not exist --fail", () => {
  return UserServices.userDelete("59e3ccd501154dcbf5f878b5")
    .then(() => {
      throw new Error(
        "Expected userDelete to fail but it succeeded"
      );
...
test("find user by _id, invalid _id --fail", () => {
  return UserServices.findUserById("fakeid")
    .then(() => {
      throw new Error(
        "Expected findUserById to fail but it succeeded"
      );
    })
    .catch((error) => {
      // Check if the error message matches the expected error
...
test("find user by username  --success", async () => {
  try {
    ...
    // Step 3: Find the added notebook by ID
    const foundUser = await UserServices.findUserByUserName(
      addedUser.username
    );
    expect(foundUser).toBeDefined();
